///   @file morphology_implementation.hpp
///   @brief  IMPLEMENTATION DETAILS.  THIS CODE IS NOT INTENDED FOR PUBLIC USE
///           (The functions defined here are invoked by "morphology.hpp")
///   @author Andrew Jewett
///   @date 2021-7-07

#ifndef _MORPHOLOGY_IMPLEMENTATION_HPP
#define _MORPHOLOGY_IMPLEMENTATION_HPP
...
{
  // NOTE:
  // C++ doesn't allow us to supply nullptr to a function that expects a pointer
...
///
/// @note    Public use of this function is allowed, but discouraged.
///          If possible, use FindMinima() or FindMaxima() instead.
...
_FindExtrema(int const image_size[3],          //!< size of the image in x,y,z directions
             ...
             vector<IntegerIndex> *pv_minima_nvoxels, //!< store number of voxels in each minima (usually 1)
             vector<IntegerIndex> *pv_maxima_nvoxels, //!< store number of voxels in each maxima (usually 1)
             Scalar minima_threshold=std::numeric_limits<Scalar>::infinity(), //!< OPTIONAL: ignore minima with intensities greater than this
             Scalar maxima_threshold=-std::numeric_limits<Scalar>::infinity(), //!< OPTIONAL: ignore maxima with intensities lower than this
             int connectivity=3,       //!< OPTIONAL: square root of search radius around each voxel (1=nearest_neighbors, 2=diagonal2D, 3=diagonal3D)
             bool allow_borders=true,  //!< OPTIONAL: if true, plateaus that touch the image border (or mask boundary) are valid extrema
             Label ***aaaiDest=nullptr,  //!< OPTIONAL: create an image showing where the extrema are?
...
template<typename Scalar, typename Coordinate, typename IntegerIndex, typename Label>
size_t
_FindExtrema(int const image_size[3],            //!< size of input image array
...
#endif //#ifndef _MORPHOLOGY_IMPLEMENTATION_HPP
