...
test_membrane_detection() {
    ...
    ../bin/filter_mrc/filter_mrc -w 19.2 -i test_image_membrane.rec -out ${OUT_FNAME_REC} -surface minima 55 -surface-tv 4 -surface-tv-angle-exponent 4 -save-progress test_image_membrane
    ../bin/filter_mrc/filter_mrc -w 19.2 -i test_image_membrane.rec -out ${OUT_FNAME_REC} -surface minima 55 -surface-tv 4 -surface-tv-angle-exponent 4 -load-progress test_image_membrane -connect 1e+09 -connect-angle 30 -surface-normals-file ${OUT_FNAME_NORMALS} -select-cluster 1 >& test_log_e.txt
